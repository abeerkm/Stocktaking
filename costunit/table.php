...
if(isset($_POST["submit"]))
{
          ...
          $sql = "insert into costunits (Item, Cost_unit_2) values
           ('".$txtitem."','".$txtcost."')";
...
$(document).ready(function(){
 ...
 // Append table with add row form on add new button click
  ...
  var index = $("table tbody tr:last-child").index();
        var row = '<tr>' +
            ...
            '<td><input type="text" class="form-control" name="Cost_unit_2" id="Cost_unit_2"></td>' +
 ...
 $(document).on("click", ".add", function(){
  ...
  var txtitem = $("#Item").val();
  var txtcost = $("#Cost_unit_2").val();
  var date = $("#last_date").val();

  $.post("add.php", { txtitem: txtitem, txtcost: txtcost,date:date}, function(data) {
 ...
 $(document).on("click", ".update", function(){
  ...
  var date = $("#date").val();
  $.post("update.php", { string: string,txtitem: txtitem, txtcost: txtcost,date:date}, function(data) {
   $("#displaymessage").html(data);
  });
  location.reload();
...
<body>
    <div class="container"><p><h1 align="center">cost units </h1><div id="displaymessage"></div></p>
...
<?php 
include"../dbcon.php"; 
$query_pag_data = "SELECT * from costunits  ";
$result_pag_data = mysqli_query($conn, $query_pag_data);
while($row = mysqli_fetch_assoc($result_pag_data)) {
    $rowid = $row['rowid']; 
    $txtitem = $row['Item']; 
 $txtcost = $row['Cost_unit_2']; 
 $date = $row['last_update'];
?>
                    <tr>
                        <td><?php echo $txtitem; ?></td>
